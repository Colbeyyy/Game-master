...
public class Camera {
	...
	public void update()
	{
		...
		//Matrix4f viewBill = Matrix4f.translate(new Vector3f(-0.2f, 0, 0)).multiply(Matrix4f.rotate(-rotation.x, new Vector3f(1, 0, 0)).multiply(Matrix4f.rotate(-rotation.y, new Vector3f(0, 1, 0)).multiply(Matrix4f.rotate(-rotation.z, new Vector3f(0, 0, 1)))));
		
		
		Shader.ENTITY.setUniformMat4f("vw_matrix", view.multiply(Matrix4f.translate(new Vector3f(-position.x, -position.y, -position.z))));
		
		//Shader.BILLBOARD.setUniformMat4f("vw_matrix", viewBill.multiply(Matrix4f.translate(new Vector3f(-position.x, -position.y, -position.z))));
